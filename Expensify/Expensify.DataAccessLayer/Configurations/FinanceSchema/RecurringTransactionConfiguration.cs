    ...
    : IEntityTypeConfiguration<RecurringTransaction>
{
    ...
    public void Configure(EntityTypeBuilder<RecurringTransaction> builder)
    {
        ...
        builder.Property(recurringTransaction => recurringTransaction.MerchantName)
            .HasMaxLength(DatabaseLengths.Name);

        // Configures optional notes.
        builder.Property(recurringTransaction => recurringTransaction.Notes)
            .HasMaxLength(DatabaseLengths.Notes);

        // Configures the recurrence frequency enum.
        builder.Property(recurringTransaction => recurringTransaction.Frequency)
            .IsRequired();
        ...
        builder.Property(recurringTransaction => recurringTransaction.NextRunDate)
            .IsRequired();
        ...
        // still reference it.
        builder.HasOne(recurringTransaction => recurringTransaction.Account)
            .WithMany()
            .HasForeignKey(recurringTransaction => recurringTransaction.AccountId)
            .HasConstraintName(
                "fk_recurring_transactions_accounts_account_id"
            )
        ...
        builder.HasOne(recurringTransaction => recurringTransaction.Budget)
            .WithMany()
            .HasForeignKey(recurringTransaction => recurringTransaction.BudgetId)
            .HasConstraintName(
                "fk_recurring_transactions_budgets_budget_id"
            )
        ...
        // the recurring transaction.
        builder.HasOne(recurringTransaction => recurringTransaction.Category)
            .WithMany()
            .HasForeignKey(recurringTransaction => recurringTransaction.CategoryId)
            .HasConstraintName(
                "fk_recurring_transactions_categories_category_id"
            )
            .OnDelete(DeleteBehavior.SetNull);
        ...
        // the recurring transaction.
        builder.HasOne(recurringTransaction => recurringTransaction.PaymentMethod)
            .WithMany()
            .HasForeignKey(recurringTransaction => recurringTransaction.PaymentMethodId)
            .HasConstraintName(
                "fk_recurring_transactions_payment_methods_payment_method_id"
            )
            .OnDelete(DeleteBehavior.SetNull);
        ...
        builder.HasIndex(recurringTransaction => recurringTransaction.AccountId)
            .HasDatabaseName(
        ...
        builder.HasIndex(recurringTransaction => new
        {
            recurringTransaction.IsActive,
            recurringTransaction.NextRunDate
        })
        .HasDatabaseName(
            "ix_recurring_transactions_is_active_next_run_date"
        );

        // Improves optional filtering by budget.
        builder.HasIndex(recurringTransaction => recurringTransaction.BudgetId)
            .HasDatabaseName(
                "ix_recurring_transactions_budget_id"
            );

        // Improves optional filtering by category.
        builder.HasIndex(recurringTransaction => recurringTransaction.CategoryId)
            .HasDatabaseName(
                "ix_recurring_transactions_category_id"
            );

        // Improves optional filtering by payment method.
        builder.HasIndex(recurringTransaction => recurringTransaction.PaymentMethodId)
            .HasDatabaseName(
                "ix_recurring_transactions_payment_method_id"
            );
    }
}